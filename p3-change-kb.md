...
**Justification :** ce changement n'est pas *standard* car il n'a jamais été appliqué auparavant sur ce système — pas de procédure pré-approuvée, répétable et à faible risque existante qui permettrait de sauter directement l'étape CAB. Il n'est pas non plus *urgent* car aucune panne en cours ne l'impose : il s'agit d'une amélioration planifiée, avec le temps nécessaire pour suivre le circuit complet (demande → évaluation du risque → CAB → autorisation → déploiement).

### Description du changement

Configuration de l'outil de ticketing (GLPI) pour :
...
- Les agents du helpdesk (changement de processus : doivent garantir que chaque ticket reste bien rattaché à son ID unique lors des transferts N1/N2)
- L'infrastructure email (volume de notifications automatiques supplémentaire à gérer)
- Le CI « portail helpdesk » recensé dans la CMDB, dont la configuration évolue
...
1. Conserver une sauvegarde de la configuration GLPI avant modification (export de la configuration actuelle, horodaté)
...
« Ce changement répond directement au symptôme le plus critique remonté par les utilisateurs : les tickets perçus comme perdus faute de suivi. L'effort de mise en œuvre est faible (paramétrage de GLPI, pas de nouveau système), et le risque est limité car réversible en moins de 15 minutes. Je demande une fenêtre de déploiement en dehors des heures de pointe pour limiter l'impact du risque de surcharge email. »

**Argumentation de l'approbateur (représentant CAB) :**
« Le changement est approuvé sous réserve de deux conditions : premièrement, réaliser un test de charge des notifications email sur un sous-ensemble de tickets avant déploiement complet, pour valider l'absence de surcharge ; deuxièmement, prévoir une communication préalable aux agents helpdesk sur le nouveau processus de transfert de ticket, pour éviter que l'ID unique ne soit pas correctement conservé lors des transferts N1/N2. Le plan de rollback proposé est jugé suffisant compte tenu du risque limité. Changement approuvé pour la prochaine fenêtre de déploiement standard. »

**Décision : Approuvé**

---

## 2. Article de base de connaissance (KEDB — base des erreurs connues)
...
### Contournement / Résolution
1. Rechercher le ticket dans GLPI par le nom de l'utilisateur ou l'horodatage approximatif de sa demande (le ticket existe bien côté système même sans notification reçue)
2. Vérifier et corriger le champ email dans la fiche du ticket
3. Déclencher manuellement un renvoi de la notification automatique depuis l'interface (bouton « renvoyer la confirmation »)
...
## 3. Positionnement dans le Product and Service Lifecycle (PSLM)

### Étapes mobilisées

Ce changement mobilise principalement deux des 8 activités du PSLM (le cycle qui remplace la chaîne de valeur du service d'ITIL 4) :

- **Build** : c'est l'étape centrale, puisqu'il s'agit de configurer la fonctionnalité de génération d'ID unique et de notification automatique sur GLPI — on construit une capacité qui n'existait pas encore techniquement.
...
Le PSLM n'impose pas de parcourir les 8 étapes dans l'ordre à chaque changement, car tous les changements ne partent pas du même point du cycle de vie du service. Dans notre cas concret :

- On saute directement à **Build** sans repasser par Discover/Design/Acquire, car le service (le helpdesk) existe déjà et GLPI est déjà en place : il ne s'agit pas de créer un nouveau service mais d'enrichir un service existant.
- **Operate**, **Deliver** et **Support** continuent de fonctionner en parallèle pendant que Build et Transition ont lieu sur ce changement précis — le helpdesk continue à traiter les tickets courants pendant qu'on développe et teste la nouvelle fonctionnalité.
