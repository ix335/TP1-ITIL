...
| 2 | `p2-slm-events.md` | Service Level Management (SLA/OLA/UC), Event Management |
| 3 | `p3-change-kb.md` | Change Enablement, Knowledge Management (KEDB), Product and Service Lifecycle (PSLM) |
...
Ce principe a guidé l'ensemble du cas, pas seulement la priorisation initiale du CSI Register (Partie 1). Exemple concret tiré du TP : en Partie 3, le CAB n'a pas simplement approuvé ou rejeté la RFC-2026-001 en bloc — il a conditionné son approbation à un test de charge préalable sur un sous-ensemble de tickets avant le déploiement complet. C'est une application directe du principe : on ne déploie pas la fonctionnalité complète d'un coup, on valide d'abord un incrément mesurable avant d'engager le reste. Le plan de rollback en moins de 15 minutes (Partie 3) sert le même objectif : permettre d'itérer sans risque disproportionné en cas de signal négatif. On retrouve la même logique dans la boucle PDCA (Plan-Do-Check-Act) qui sous-tend le Continual Improvement : sans indicateur mesuré à l'étape Check, l'itération n'a rien à valider.
...
En revanche, l'amélioration n°3 du CSI Register (Partie 1 — SLA par catégorie et dashboard de charge avec alertes) ouvre une piste légitime : si l'on voulait automatiser la **priorisation initiale d'un ticket** à partir de sa description en texte libre — plutôt que de laisser l'agent calculer manuellement Impact × Urgence, source potentielle de la lenteur constatée — on mobiliserait la capacité **Cognition** du modèle 6C (classification/prédiction). Dans ce cas précis, le module AI Governance aurait un apport concret et non théorique :

- **Contrôle humain sur les tickets critiques (P1)** : la classification automatique ne devrait jamais déclasser ou reclasser un ticket P1 sans validation humaine explicite, pour éviter qu'une erreur de modèle ne retarde un incident critique.
